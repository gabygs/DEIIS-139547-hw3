...
public class CASconsumer extends CasConsumer_ImplBase {
	...
	public void processCas(CAS aCAS) throws ResourceProcessException {
		...
		AnnotationIndex<Annotation> precScAnnot = aJCas.getAnnotationIndex(Precision.type);
		FSIterator<Annotation> precScIt = precScAnnot.iterator();
		
		while(anwScIt.hasNext()){
			anwSc =(AnswerScore)anwScIt.next();
			System.out.println("Answer Score - CAS Consumer: "+ anwSc.getScore());
		}
		while(precScIt.hasNext()){
			precSc =(Precision)precScIt.next();
			System.out.println("PRECISION - CAS Consumer: "+ precSc.getPrecision());
		}
